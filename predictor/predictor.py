...
class Predictor():
    ...
    def predict(self, content):
        ...
        accu_result = self.session.run(self.model.y_pred, feed_dict=feed_dict)

        saver.restore(sess=self.session, save_path=self.year_model)
        year_result = self.session.run(self.model.y_pred, feed_dict=feed_dict)
        ...
        for i in range(len(content)):
            temp = {"accusation": accu_result[i]+1, "relevant_articles":
                    relvvant_result[i]+1, "term_of_imprisonment": year_result[i]}
            result.append(temp)
        return result
